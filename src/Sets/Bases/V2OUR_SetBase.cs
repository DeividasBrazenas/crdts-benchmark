...
{
    public abstract class V2OUR_SetBase<T> where T : DistributedEntity
    {
        ...
        protected V2OUR_SetBase()
        {
            Adds = ImmutableHashSet<OUR_SetElement<T>>.Empty;
            Removes = ImmutableHashSet<OUR_SetElement<T>>.Empty;
        }

        protected V2OUR_SetBase(IImmutableSet<OUR_SetElement<T>> adds, IImmutableSet<OUR_SetElement<T>> removes)
        {
            Adds = adds;
            Removes = removes;
        }
